...
```text
โจทย์ input เป็น เลขจำนวนเต็มบวก และให้ output คือ เลขที่หารด้วยสิบลงตัว ใน input ที่ให้มา ไล่ไปเรื่อยๆ จนถึง 0 แบบ n n-10 n-10*2 n-10*3 ... 10 0 
```
...
```text
รับข้อมูลเข้ามาเก็บในตัวแปร n 
จากนนั้น กำหนด ให้  n= n//10  แล้ว * 10 จะได้  จำนวนที่มากสุดที่หารสิบ ลงตัวแล้ว 
ทีนี้ผมกำหนดตัวแปรใหม่คือ out = "" จากนั้นตั้ง while ให้วน แล้วเเพิ่มค่า str(n) + " " ในout
และ  n -= 10 ไปเรื่อยๆ จนถึง 0 สุดท้าย ค่อยปริ้น out.strip() เพื่อให้ output ถูกต้องตามโจทย์
```
...
```text
ก็ ทำเหมือน แผนแรกเลยครับ รับค่า สร้างตัวแปร เพิ่มค่าสตริงเข้าในตัวแปรซ้ำๆ แล้วใช้ สปลิท ตอนท้ายเพื่อปริ้น ให้out[put ตรงกับโจทย์
```
...
```text
ทดลองว่า จะมี " " เพิ่มมาท้าย output ไหม
```
...
```text
ทดลองว่าการ ที่เลข ตั้งต้นหารสิบไม่ลงตัวส่งผลอะไรไหม
```
