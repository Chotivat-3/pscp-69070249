...
```text
ตัวโจทย์ เนี่ยต้องการให้เราเขียนโปรแกรมเพื่อบอกว่าในอีกกี่วิบ้านนี้น้ำจะท่วม โดยกำหนดจุดเริ่มต้นน้ำหมึกอยู่ที่ จุด 0,0 [start point] และ น้ำหมึกจะแผ่ออกไปในวงกลม ประมาณนั้นครับ ทีนี้ 
ตัว input จะมี 1. พื้นที่ที่น้ำหมึกแพร่ไป ต่อ 1 หน่วย วินาที และ จำนวนบ้าน [S N]
2. จะเพิ่มเรื่อยๆ ตามจำนวน N ก็ตัวนี้เป็น พิกัดที่จะให้เช็ตว่า จะโดนท่วมในอีกกี่วินาที
โดยโจทย์ กำหนดว่าถ้าเกิด วินาทีเนี่ย เมีเศษ มีจุด เช่น 1.xx ให้ปัดเปป็น 2 ไปเลย  
```
...
```text
ครับ ทีนี้มาเริ่ม จาก จัดการตัว input ที่รับเข้ามา บรรทัดแรกก่อน 
ผมใช้วิธี กำหนดตัวแปร รับค่า input().split() จากนั้นกลายเป็น list 
และค่อยเรียกค่า s = int(list[0]) ... เก็บค่า s n แบบอื่นๆแบบนี้ 
ต่อมา มาที่การคำนวณต่างๆ 
จากที่เรารู้ พื้นที่ ที่เพิ่มขึ้นต่อวินาที แต่ ที่เราจะเอาไปเทียบกับ พิกัด มันควรจะเป็น เส้นตรงๆ
ผมจึงคิดว่าหารัศมี วงกลมน่าจะดี 
จะได้ว่า r = s/pi ปล. อันนี้ คือ r ต่อวินาที
ทีนี้ก็ไปดูต่อ ที่ บ้้านต่างๆ 
ขอกำหนดจุดที่ได้มา เป็น x, y
ทีนี้ จากการเทียบระยะห่าวระหว่างสองจุดด้วย พิธาโกรัส จะได้ว่า
dis = x^2 + y^2 ปล.เพราะจุดเริ่ม = 0, 0 ดังนั้น delta x, y = x, y
ท่ากับว่าเราจะได้นะยะทางมาแล้ววว 
ทีนี้ตอนนี้เรามีความเร็ว คือ พ ต่อวินาที และระยะทาง เราจะหาเวลา
จาก v = s/t --> r = dis/t ---> t = dis/r
ทีนี้จากโจทย์บอก่าถ้ามีจุดให้ปัดขึ้น ผมใช้เป็น การหาร แบบ // แล้วก็ตั้ง if % ไว้ ถ้ามีเศษให้ t+=1
print ออก แล้วก็ในช่วงของบ้านผมใช้เป็น while loop 
ตั้งแบบ while n :
--------- code ---------(โค้ดการคำนวนด้านบน)
 n -= 1
...
```text
เหมือนกับแผนแรกครับ รับค่า มา แยกด้วย split ใช้ while ครอบ code พาร์ทการคำนวน 
และปริ้นออกทีละตัว โดยการคำนวน ใช้ตัว 
การคำนวณ v = s/t โดยตรงนี้กำหนดให้ v = r และ s = dis 
ระยทางจากจุด 0, 0 ถึงบ้าน แต่ละหลัง มีการใช้ // และ if % ในการปัดขึ้น ครับ
```
...
```text
ลองดูว่า กรณีเป็นทศนิยม จะปัดขึ้นไหม
```
