...
```text
โจทย์ ให้คำนวณเงินที่ต้องจ่าย โดย มีค่าบริการ 10% (ขั้นต่ำ 50 แปลว่าถ้า 10%น้อยกว่า50บาทจะล็อกไว้ที่ต่ำสุด 50 และ ไม่เกิน 1000 ) vat 7%(ค่าอาหารรวมกับค่าบริการแล้วคูณด้วย 7%) จากนั้นก็จะได้ มาเป็น ค่าที่ต้องจ่ายคือ food + service + vat 
input = ค่าอาหาร
output = เงินที่ต้องจ่าย(ปล.จน.จริงทศนิยม 2 หลัก)  
```
...
```text
ตอนแรก ผมลองเขียนสรุปคร่าวๆก่อนว่าต้องมีค่าอะไรบ้าง ได้ว่า มีค่าอาหาร ค่าบริการ และ vat ซึ่ง ต้องคิดค่าอาหารและบริการก่อนจึงไปคิด vat ผมเริ่งจากเขียน ไว้ว่าแต่ละอันคำนวณยังไง จากนั้นก็ลงโค้ด 
รับค่า food 
คำนวน ค่า service = food*(10/100)
ค่า vat = (food + service)*7%
จากนั้นตั้งเงื่อนไข 
if food > = 10000: service = 10000
else:
     service = food*(10/100)
     if service  < 50 :
            service  = 50
      vat = ....
จากนั้น overall = food + service + vat
overall = round(overall,2) ปัดตามหลักคณิตศาสตร์
print(f"{overall:.2f}") ปริ้น แบบ 2 ตำแหน่ง
...
```text
เหมือนแผนแรกเลยครับ เนื่อจากลองทดสอบตามตามเงื่อนไขต่างๆ แล้วจากรุปแบบการคำนวณผมคิดว่าไม่น่าจะผิดพลาดแล้วครับ
```
...
```text
กรณีที่ ผลลัพธ์ ออกเป็นเลขจำนวณเต็ม
```
...
```text
กรณี ที่ ค่าบริการ เท่ากับ 1000
```
