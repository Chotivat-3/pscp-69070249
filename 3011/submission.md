...
```text
โจทย์ พูดถึงการผสมสีของแม่สี โดยที่ จะให้ input มาเป็น สี 2 สี ซึ่ง จะเป็นแม่สีหรือไม่ก็ได้ จากนั้นให้เรา output เป็น สี ที่ได้ออกมาหรือหาไม่ใช่แม่สีก็ให้ Error โดยโจทย์กำหนดให้มีแม่สี คือ แดง เหลือง น้ำเงิน แสดงว่าหากมีสีนอกจากนี้มาก็จะ Error
```
...
```text
ตอนแรกที่ผมทำ ผมพยายามจะรับค่าสีเข้ามาสองค่า และเอาไปใส่ใน list ทำไมไม่รู้เหมือนอยากใช้ listเฉยๆ ตรงนี้ผมใช้ 
c = []
c.append(input())
c.append(input())
color = ""
c1 = c[0]
c2 = c[1]
 ผมก็กำหนดค่า ว่า ตัวแปร1,2 เท่ากับตัวที่ 1 และ สองใน list และ ผมใช้ for loop ในการ ตรวจสอบว่า ใน list นี้ มีตัวอื่นนอกจาก แม่สี ให้ เก็บค่าว่าError ไว้และ ปริ้นออก  ทว่าผมใช้คำสั่งที่ว่า
for i in c :
        if i != "Red" or i != "Yellow" or i != "Blue":
            color = "Error"  เท่ากับว่าพอเช็คเงื่อนไขว่า Error แล้ว จะเก็ยค่าไว้ใน color ทว่า
จากนั้นผมใช้ if ในการ ตั้งเงื่อนไขสีต่างๆ ทีนี้ ผมเรืิ่มด้วย ถ้าสีเหมือนกันให้ปริ้นสีนั้นมาเลย
if x1 == x2 :
        color  = f"{x1}"
    else: ทว่ามันดันมาผิดตรงนี้นี่ล่ะครับ ผมตั้งแบบนี้
match x1:
            case "Red":
                if x2 == "Yellow":
                    color  = "Orange"
                    ...
                    color  = "Violet"
ตอนแรกที่มันเก็บค่า Error ไว้ ใน color ก็ไม่มีผลอะไร  ถึงแม้ตัวที่ 2 จะเป็น สีอื่นนอกจากแม่สี ยังไงมันก็จะลงค่า color = Violet เพราะผมใช้ else  สุดท้ายตอน output ออกมาก็ผิดครับ
...
```text
ปล.ตอนแรกผมใช้ match case ไล่ทีละเงื่อนไข ละ ติด prep8 เนื่องจากเขียนเงื่อนไขเยอะไป จากนั้น ผมส่งแล้วไปดูของเพื่อนๆ ผมเห็นเขาใช้ list ในการเก็บค่าแม่สี และ จับคู่เงื่อนไขในการตรวจสอบสีเป็นคู่ๆทำให้ลดบรรทัดและเงื่อนไขลงได้เยอะครับ 
```
...
```text
กรณีที่ ตัวแรกเป็นแม่สี และ ตัวที่สองไม่ใช่แม่สี
```
...
```text
กรณีที่ ทั้งสองสีเหมือนกัน แต่ไม่ใช่แม่สี
```
...
```text
ไม่ได้ถามเพื่อนแต่ได้ดูแนวโค้ดจากเพื่อนใน 
i judge ครับ
```
...
```text
แนวทางในการเก็บค่าแม่สีไว้ใน list และ ดึงข้อมูลมาใช้ครับ
```

คุณยังทำอะไรด้วยตนเอง

```text
การเขียนโค้ด ในส่วนที่ดึงข้อมูลมาใช้ในแต่ละเงื่อนไขผมเขียนในแบบของผมครับ 
```
