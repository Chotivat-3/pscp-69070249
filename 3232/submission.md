...
```text
โจทย์ ข้อนี้พูดถึงการ คำนวนระยะทางที่เพิ่มขึ้นโดยค่าที่ลดลงอย่างคงที่ 
โดยเปรียบเทียบกับการกระโดด และ ต้องการทราบว่า สุดท้ายแล้ว ระยะที่ได้จะ 
พอที่จะไปถึง ระยะกำหนดไหม โดย โจทย์จะให้ จุดหมายมา และระยะการ กระโดดมา 
โดย มีเงื่อไข ว่าระยะการกระโดด จะลดลง ครั้ง 2 หน่วยทุกๆๆครั้ง
ถ้าเกิดถึงจุดหมายให้ปริ้นจำนวน การกระโดด
ถ้าไม่ถึงให้ปริ้น -1
```
...
```text
โดย อย่างแรก ผมรับ ค่า ระยะกระโดด จุด หมายเข้ามาก่อน (x, y)
จากนั้น ก็อย่างแรก ผม กำหนดค่าระยะ ทาง การกระโดดครั้ง แรก ให้ เท่ากับ x
และ สร้างตัว แปร n = 1 ขึ้นมาเก็บจำนวนครั้งที่กระโดด
ต่อมา ผมใช้ while True : ในการ วนลูป เช็คระยะ โดย ตั้งเงื่อนไข ไว้ บนสุดว่า
ถ้า dis > y ก็คือถ้าระยะกระโดดมากกว่า y ให้ print(n) และ break loop เลย
เงื่อไข ต่อ ไปคือ ถ้า x <= 0 ให้ print(-1) ก็คือ ถ้ามันผ่านเงื่อนไขแรกมาแล้วระยะ ยังไม่มากกว่า
แต่ระยะที่กระโดด ได้ ดันเหลือ ไม่มีแล้วแปลว่า กระโดด ไม่ถึงที่หมายแน่นนอน ดังนั้น print(-1)
สุดท้าย ล่างสุด x-=2, dis += x, n+= 1ก็คือ ระยะกระโดดลดลงทีละ 2 และ ระยะที่กระโดเพิ่มทีละ x
```
...
```text
ก็ตามวิธีแรกเลยครับ รับค่าเข้ามาจัดตัวแปร สร้าง loop สร้าง เงื่อนไขต่างๆ เพื่อ กระโดด 
และเทียบค่า จากนั้น ปริ้นออกตาม ที่โจทย์กำหนก คือ จำนวนการกระโดด หรือ -1 
```
