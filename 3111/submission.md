...
```text
โจทย์ ข้อนี้ พูดถึงเรื่องการคำนวณยอดรวม จาก เงื่อนไขต่างๆ
โดย จะมีเงื่อนไข คือ 
ถ้าเป็นสมาชิก ให้ ลด 5 เปอร์เซ็น
ถ้าไม่เป็นแต่ ซื้อเกิน 500 บาท ก็ให้ลด 3 เปอร์เซ็น
แล้วก็รับ จำนวนราคาตามจำนวนสินค้าที่ซื้อ

สุดท้ายให้ เอ้าพุท ออกมาเป็น ยอดรวม ทศนิยม 2 ตำแหน่ง โดย ที่ให้ปัดขึ้น ตามหลักปัดเศษ ปกติ
```
...
```text
ในตอนแรก ผม ก็รับค่า จำนวน สินค้า รับค่า ราคา จากนั้น ก็รวม และตั้ง if elif เพื่อ ดูเงื่อนไขต่างๆ
ถ้า เกิดเป็นสมาชิก ให้ ยอดรวม ลดลง 5 เปอร์เซ็น แต่ถ้าไม่เป็นและซื้อเกิน 500 ให้ ลด 3 เปอร์เซ็น
ทีนี้มันติดปัญหาว่า ผมปัดเสษได้ไม่เหมือนใน sample ตอนแรก ผมใช้ round จากนั้น ผมก็เปลี่ยนเป็น
การ ให้ ผลลัพธ์ที่ได้*100 + 0.5 และ ทั้งหมด หารด้วย 100 ทว่าก็ยังไม่ได้อยู่ดี ตรงนี้ผมคิดว่า ปัญหาอยู่ที่ float ผมเลยไม่รู้จะแก้ยังไง...
```
...
```text
จากนั้น ผมเลย ไปลองหาวิะีเพิ่มเติม จน ได้รู้จักกับ ตัว decimal ที่สามารับ ค่า ตัวเลข เป็น string 
และนำไปคำนวณได้ ผมจึงได้ import Decimal , ROUND_HALF_UP จาก decimal 
โดย ตรงนี้ ผมเลยรับ ค่า เข้ามา เป็น Decimal เลย จากนั้น นำไปคำนวณ ตั้งเงื่อนไขต่างๆตามเดิม 
เปลี่ยนตรงสุด ท้าย ผมได้มี การ ใช้ x.quantize(Decimal("0.01"),rounding=ROUND_HALF_UP)
โดย ตัว .quantize(Decimal(กำหนดจุด, rounding=บอกว่าจะปัดแบบไหน)
จากนั้น ก็ ปริ้น ออกได้ ครับ
```
...
N
2
10.498566921325623231566232563322333
20
```
