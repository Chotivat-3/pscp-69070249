...
```text
โจทย์ต้องการให้เรา แสดงผลชื่อไพ่ออกมาโดยที่จะให้รับค่า อินพุทเป็นตัวย่อ
โดยจะมีไพ่ 4 ชนิด และ ไพ่จะมีค่าเปนแต้ม 2 - 10 หรือ เอช แจค ควีน คิง
เมื่อรับ อินพุทเข้ามาเราแค่แสดงผลออกไปให้ตรงตามที่โจทย์ต้องการ
```
...
```text
ผมคิดว่า จะรับค่าข้อความเข้ามาเก็บไว้ก่อนจากนั้น
ผมสร้าง dict สำหรับเก็บข้อมูลไว้ สอง อัน
 คือ 1 ข้อมูลชนิด คือ tp  2 ข้อมูลคลาส คือ cl
จากนั้น ผมก็มาสร้างเงื่อนไขเช็คว่า text ที่รับมานั้น เป็นประเภทอะไร
โดย การ ใช้ if เช็ค
เงื่อนไข แรก ที่ผม เช็คคือ ตัวแรกเป็นเลข 2-9 ไหม
ถ้าใช่ก็ให้ กำหนด ค่า text[0],[1] ลง ตัวแปร และ เรียกค่าออกจากก dict พร้อมจัดรูป เก็บค่าไว้
ต่อมาก็เช็ค ถ้าขึ้นต้นด้วย 1 แปลว่า คือ 10 
ทำเหมือนเดิมแต่แันนี้ เลข ข้างหน้า ใช้ string slicing ในการกำหนด 
จากนั้น ก็ ใช้ t[2]เรียกค่าจาก dict ต่อม่ก็ใช้ f string จัดรูปเก็บไว้
เงื่อนไขสุดท้าย คือ เป็น เอช แจ็ค ควีน คิง ก็ทำเหมือน if แรก แค่เรียกค่าใน dict  ทั้ง สองตัว
เป็น dict ของ type และ class จากนั้นจัดรูปเก็บไว้ตัวแปร
ปิ้นตัวแปรที่เก็บค่าที่จัดไว้ออกมา
```
...
```text
ทำเหมือนวิธีแรกเลยครับ คือ รับค่า เข้ามา สร้างดิค เก็บข้อมูล สร้าง if ดู เงื่อนไขต่างๆ
จัดรูปเก็บไว้ในตัวแปร ปริ้นออกมาครับ
```
...
```text
ทดลองการดึงค่า จาก dict ทั้ง 2
```
...
```text
ทดลองดูว่าหากตัวแรกเป็นเลข 10 จะยังครอบคลุมไหม
```
