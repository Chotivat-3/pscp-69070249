...
```text
ตัวโจทย์เนี่ย ให้ตารางรูปแบบที่มีการลดขั้นลงไปเรื่อยๆ เปรียเสมือนแปรงดอกไม้มาให้เรา
และแยกออกเป็นแถบๆ ตามรูปภาพและโจทย์ต้องการถามว่าถ้าเราปลูกไป N ช่องเนี่ย จะมีกี่แถบ
```
...
```text
ในตอนแรก ผมแยกพาร์ทออกมาเป็น 2 พาร์ท คือ ส่วนเหลือ และส่วนเต็ม
โดยที่ สุ่นเหลือ คือ ส่วนปลาย ที่ มี ขนาดเท่ากับแถบแรกของทุกขนาดแปลง เช่น 1 ก็จะเหลือ 1
2 ก็จะเหลือ 3 3 ก็จะ เหลือ 6 เป็นการใช้ ลำดับเลขคณิตในการคำนวณ ทีนี้ ผมจะคำนวนทีละแถบ
โดย กำหนดตัวแปรมารับค่าไว้ก่อน ใ้ห้ เป็น total โดย ที่  total จะ+ค่า Sn = l/2(l+1)
จากนั้นในถบต่อๆไปผมใช้ loop ในการคิด โดย ที่จะมี แพทเทริน ที่ว่า ในแต่ละ แถบจะมีแถวที่มีจำนวน
l เต็ม 2 แถว และจะเพิ่มขึ้นเรื่อยๆ ไปทีละเท่าตัวตามแถบที่เพิ่ม ทีนี้ ตัวผมเลยให้ ค่า แถบ แต่ละรอบเนี่
เท่ากับ ค่าของแถว เต็ม + กับ re และ นำค่านี้ไป + ที่ total เรื่อยๆ จนกว่าจะมากกว่าหรือเท่ากับ n
```
...
```text
เป็นการพัฒนามาจากโค้ดแรกครับ จากการมองแยกเป็นการมองรวมแทน
ผมมองเห็นว่าแต่ละแถบจะมีขนาดของหลักแรกเท่ากับ จำนวนแถบคูณด้วย l
และจำนวนช่องทั้งหมดมันเท่ากับผลรวมของจำนวนแถวในแต่ละหลักลบ1ไปเรื่อยๆ จนถึง 1
ดังนั้น ผมจึงไปใช้สูตร Sn ในการคำนวน ช่องรวมทั้งหมดทีเดียวเลย
โดย ที่ เริ่ม กำหนดค่า  t เป็น ค่า ของแถบ ต่อมาผมใช้ While True ในการดำเนินการไปเรื่อยๆ
ผมกำหนดตัวแปล bay = (l*(t+1)/2)*(l*(t+1)+1) โดย อันนี้ คือ สูตร Sn ในการคำนวนจำนวนช่อง
โดย จะเป็นการคำนวนช่องทั้งหมด เมื่อนเราอยุ่ใน แถบที่ t ดังนั้น ผมจึงตั้งว่า
ถ้า bay เนี่ย มากกว่า n ให้ปริ้น t ออกมาและ break 
โดย ตัวสุตรนะครับคือมาจาก sn = n\2(a1+an) โดยที่  n,a1 = l*จำนวนแถบ และ an = 1
```
