...
```text
ก็คือ ตัวโจทย์ ข้อนี้ต้องการ ให้เรา เช้ค ว่า เวลาที่ให้มาเนี่ย  แต่ละ เวลา มีร้าน เปิดกี่ ร้าน 
โดย รู้สึกว่าโจทย์ นี้น่าจะเล่นกัยตัว input และการกำหนดช่วงครับ
โดย ที่ input แรก เป็น ค่า จำนวนช่วงเวลาร้าน และ จำนวน เวลาที่จะเช็ค
จากนั้นให้รับ ทั้ง 2 อย่างตามจำนวนที่กำหนด และ เอ้า พุท ออกมาเป็น บรรทัดเดียว
```
...
```text
ในโจทย์ ข้อนี้ ตอน แรก ผม รับ input เข้ามา จากนั้นใช้ map,int+split รับค่า เข้ามาในตัวแปร 2  ตัว
กำหนด เปน ns และ nc ไปก่อนนะครับ จากนั้น ผมสร้างลิสต์ ไว้เก็บ ค่าของ ช่วงเวลาเปิดร้าน
ต่กนั้นใช้ ลูป เพื่อรับค่า ต่อมา ใน ช่วงเวลาที่จะตรวจสอบทั้งหมด ใช้ การ 
กำหนดตัวแปล =list(map,int + split) เพื่อเก็บ ไว้เป็น list  ของ ตัวเลข ต่อมานั้น 
สร้าง list อีกก สอง อัน อัน แรก สำหรับ ใส่ เรนจ์ช่วงเวลา อีกอันสำหรับ out put
ต่อมา ผมได้ใช้ loop กับ ลิสที่รับเวลาเข้ามาตอนแรก i ใน list จะเป็น [เวลา เวลา] ทำให้ผมสามารถ
ทำเป็น x, y = map,int+split เพื่อที่ผมจะนำ xy ไปทำเป็นเรนจ์ xy 
และส่งค่าไปในลิส สำหรับใส่ช่วงเวลาที่เตรียมไว้

ต่อมา ได้ สร้าง ลูป อีกรอบ นึง สำหรับ เช็ค เวลาที่จะไป กัย เรนจ์เวลาแต่ละร้าน
แต่ลูป นี้ ผม ใช้ เป็นลุปซ้อนลูป ในลูปแรก ใช้ เป็น i in listที่เก็บเรนจ์เวลาไว้
ส่วนลูปใน เป็น j in range nc[จำนวนเวลาที่ต้องการเช็ค]
จากนั้นก็ตั้ง if list(เวลาที่จจะเช็ค)[j] in i: out[j]+=1 เป็นการบอกว่ามี 1 ร้าน ที่เปิดตอนนนี้
ก็คือตรงนี้ ใช้ลูปสำหรับ เช็คทีละร้าน กับทุกเวลา จนครบทุกร้าน 

จากนั้น ใช้ print(" ".join(listoutput) ) เพื่อ out put ออกมา
```
...
```text
เหมือนกับวิธีแรก ครับ เพียงแต่มีการ ตัแต่งรูปแบบการเขียนต่างๆเล้กน้อยใหถุกหลัก มากขึ้น
ก็ คือ ในข้อนี้ รับ input เข้ามา ใช้ การ list int map loop ต่างๆ
 เพื่อให้สามารถกำหนดค่า ช่วงเวลา และ เวลาที่ต้องการตรวจสอบได้ จากนั้นก็ปริ้นออกมาครับ
```
...
```text
ทดลอง ว่าโค้ดแสดงผลออกมาได้ตามโจทย์ไหม
```
...
```text
ทดลอง ว่า ช่วงเรนจ์ ที่กำหนดโอเค ไหม ตรวจสอบ ครบไหม
```
...
```text
เวลา เช็คเวลาเดียวกันย ผลลัพธ์จะเหมือนกันไหม
```
